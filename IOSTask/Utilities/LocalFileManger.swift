...
class LocalFileManger {
    ...
    private func getURLForImage(imageName: String, folderName: String) -> URL? {
        guard let folderURL = getUrlForFolder(folderName: folderName), let range = imageName.range(of: "/images/") else {return nil}
        let imagePath = String(imageName[range.upperBound...])
        let cleanImagePath = imagePath.replacingOccurrences(of: "[^a-zA-Z0-9.]", with: "", options: .regularExpression)
        return folderURL.appendingPathComponent(cleanImagePath)
    }
}
